...
Checkout.prototype = {
    initialize: function(accordion, urls){
        ...
        this.accordion.sections.each(function(section, index) {
            Event.observe($(section).getOffsetParent().getOffsetParent().select('.opc-titles .step-title-box')[index], 'click', this._onSectionClick.bindAsEventListener(this, index));
        }.bind(this));
     ...
     * @param event
     */
    _onSectionClick: function(event, index2) {
        var section = $(Event.element(event).up().up().up().select('#checkoutSteps .section')[index2]);
